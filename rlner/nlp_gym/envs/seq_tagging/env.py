...
# rlner libraries
from rlner.nlp_gym.core_components.sampler import PrioritySampler
from rlner.nlp_gym.data_pools.base import Sample
from rlner.nlp_gym.envs.common.action_space import ActionSpace
from rlner.nlp_gym.envs.common.base_env import BaseEnv
from rlner.nlp_gym.envs.common.reward import RewardFunction
from rlner.nlp_gym.envs.seq_tagging.featurizer import DefaultFeaturizerForSeqTagging
...
class SeqTagEnv(BaseEnv):
    ...
    def __init__(
        ...
        if return_obs_as_vector:
            observation_featurizer = (
                DefaultFeaturizerForSeqTagging(self.action_space)
    ...
    def step(self, action: int) -> Tuple[Union[Observation, np.array], int, bool, dict]:
        ...
        observation_to_return = (
            self.current_sample.observation.get_vector().numpy()
    ...
    def reset(self, sample: Sample = None) -> Union[Observation, np.array]:
        ...
        observation = Observation.build(
            self.time_step,
            input_text_tokens[self.time_step],
            [],
            self.observation_featurizer,
            self.return_obs_as_vector,
        )
        ...
        self.current_sample = DataPoint(text=input_text_tokens, label=sample.oracle_label, observation=observation)
    ...
    def add_sample(self, sample: Sample, weight: int = 1.0):
        """Add sample to environment"""
        self.sampler_for_replaying.add(sample, weight)

    def get_samples(self) -> List[Sample]:
        """Get samples from environment"""
